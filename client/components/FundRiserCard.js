...
const FundRiserCard = ({ props, pushWithdrawRequests, triggerRefresh }) => {
  ...
  return (
    <div className="relative w-full max-w-md p-6 pt-12 bg-white/70 backdrop-blur-md border border-gray-300 rounded-2xl shadow hover:shadow-xl transition-all">
      <div
        className={`absolute top-4 right-4 z-10 text-white text-xs font-bold px-3 py-1 rounded-full ${colorMaker(props.state)}`}
      >
        {props.state}
      </div>
